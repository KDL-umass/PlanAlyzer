...
module Make(Cpo : sig 
    ...
    module SMTAux = struct
      ...
      let get_pred_assignment solver pred_term = 
        ...
        | SMT.Sat -> 
          Logger.print_debug __LINE__ "%s is SAT" term_string;
          Some (SMT.get_model solver)
      ...
      (* Loops through the assignments in pred_model and assert the definitions
         ...
         Note that, at this time, we are only looking for unique solutions to 
         models. This means that disjunction, as depicted above, will be left
         symbolic. We can try to concretize as many variables as possible, and 
         stop when we reach the first variable that has more than one satisfying
         assignment.
      ...
      let pred_assert_references solver avoid s sphis dt dtphis dt_temp pred_model = 
        ...
        for i=0 to List.length pred_model - 1 do 
          ...
          (* Check whether we have previously concretized as a treatment or 
             as part of the conditioning set. *)
          match previously_concretized dt, previously_concretized s with 
            ...
            | Some _, Some _ -> 
              ...
              assert false 
            | None, None -> 
              Logger.print_debug __LINE__ "%s has not previously been concretized"
                (Config.string_of_ssa_id k1);
              let phis = PHelp.union sphis dtphis in 
              if PHelp.has_key k1 phis
              then 
                (* Concretize this variable and assert its references.
                   At this time, we are only looking for unique solutions. 
                   This means that disjunctions will cause problems. 
                   Need to backtrack accurately. *)

                (* Initialize the list of SMT expressions to assert with the 
                   predicate we are starting with (k1) *)
                let to_assert = ref [k1] in
                while List.length !to_assert <> 0 do
                  let assert_me = List.hd !to_assert in 
                  (* First check to see if we have the current predicate in our 
                     list of non-concretized expressions. We are guaranteed to 
                     execute the consequent of the following if-statement on our
                     first iteration through to_assert. *)
                  if PHelp.has_key assert_me phis 
                  then 
                    (* Get the expression to which k2 refers. *)
                    let e = PHelp.get_expr assert_me phis in 
                    if Cpo.is_opaque e || Cpo.is_disjunction e
                    then 
                      (* If the expression is opaque, or if it is a disjunction,
                         we can't do anything about it. Update to_assert and 
                         move on. *)
                         to_assert := List.tl !to_assert
                    ...
                    then begin
                      ...
                      Logger.print_debug 
                        ...
                        (Config.string_of_ssa_id assert_me) 
                    ...
                    else to_assert := List.tl !to_assert; 
                  else to_assert := List.tl !to_assert; 
                done;
              else ()
        done;
    ...
    let generate_treatments cfg s e k cphis dt_list_new (d, t, tphis) = 
      Logger.print_debug __LINE__ "HERE (%s)" (Config.string_of_ssa_id k);
      let env = 
      ...
      Logger.print_debug __LINE__ "Num treatments for %s: %d"
        (Config.string_of_ssa_id k)
        num_worlds;
    ...
    let eval_assign solver declared cfg path dt_list o n s cphis k q e = 
      ...
      Logger.print_debug __LINE__
        "%s\n\tis random: %b\n\tis final: %b\n\tis opaque: %b\n\tNum treatments so far: %d" 
        (Format.string_of_node n) rand final opaque (List.length dt_list);
      ...
      then begin
        ...
        let var_str = (Config.string_of_ssa_id k) in 
        Logger.print_info __LINE__
          "%s is randomly assigned and therefore a potential treatment" var_str; 
        Logger.print_debug __LINE__
          "Current number of treatments before evaluating %s: %d" var_str
          (List.length dt_list);
        let new_treatments = 
          List.fold_left (generate_treatments cfg s e k cphis) [] dt_list in 
        Logger.print_debug __LINE__
          "Number of treatments after evaluating %s: %d" var_str
          (List.length new_treatments);
    ...
    let eval_path ~pathid solver cfg (p : pi) : ((tmt list) * cndst) list =
      Logger.print_info __LINE__ "Evaluating path %d" pathid;
      SMT.push solver;
      ...
      let evaled = List.fold_left (fun tupe_list (n : node) -> 
        ...
        tupe_list
        |> Utils.enumerate
        |> List.map (fun (id, (dt_list, (s, phis))) ->
            Logger.print_info __LINE__
              "Evaluating node %s for semantic tuple %d" 
              (Format.string_of_node n)
              id;
            eval_node solver avoid declared cfg p dt_list s phis n) 
          |> List.flatten)
    ...
    let eval cfg g (p: program) : semantic_tuple list = 
      ...
      let logged_paths_evaled = logged_paths
        |> Utils.enumerate
        |> List.map (fun (pathid, p) -> 
            (p, eval_path ~pathid solver cfg p))  in
      SMT.pop solver;
