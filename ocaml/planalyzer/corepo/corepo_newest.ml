...
module Make(Cpo : sig 
    ...
    let empty_tmt = Ctxt.empty, Oratio.neg_one, PHelp.empty
    ...
    module LabelMap = struct 
      ...
      include M 
      let find_rand k m = 
        (* todo: fix this -- it happens for cluster random assignment. *)
        if mem k m then (find k m).rand else Config.Det
        (* (find k m).rand *)
    end 
    ...
    let eval_assert solver (avoid : Avoid.t) declared path pred s phis treatment  
     : (tmt list * cndst) option = 
      ...
      SMT.pop solver;
      let treatments = !tr_list 
        |> List.map (fun (tr, dt_phis) -> (tr, r, dt_phis))
       in 
      Some (treatments, (!cset, !sphis))

    let generate_treatments cfg s e k cphis dt_list_new (seed: tmt option) = 
      Logger.print_debug __LINE__ "Generating treatments for %s" (Cpo.string_of_cpo_expr e);
      let d, t, tphis = match seed with 
        | Some (d, t, tphis) -> d, t, tphis
        | None -> empty_tmt
      in 
      let env = 
        DataStructOps.merge_delta_sigma_phi d s (PHelp.merge cphis tphis) in 
      let e = Cpo.eval_cpo_expr env e in 
      Logger.print_debug __LINE__ "AST of evaled input expression:\n%s" (Cpo.ast_string_of_inlang_expr e);
      let worlds = Cpo.randomly_split_worlds cfg e in
      Logger.print_info __LINE__ "Num worlds for treatment: %d"  (List.length worlds);
    ...
    let eval_assign solver declared cfg path dt_list o n s cphis k q e = 
      ...
      then begin
        ...
        Logger.print_debug __LINE__ "Size of dt_list: %d" (List.length dt_list);
        let new_treatments : tmt list = begin
            if List.length dt_list > 0
            then dt_list
              |> List.map (fun thing -> Some thing)
              |> List.fold_left 
                (generate_treatments cfg s e k cphis) []
            else generate_treatments cfg s e k cphis [] None
          end in
        Logger.print_info __LINE__ 
          "Num treatments: %d" (List.length new_treatments);
    ...
    let ctxt_equals s1 s2 =
      if Ctxt.cardinal s1 <> Ctxt.cardinal s2
      then false 
      else 
        Ctxt.bindings s1 
        |> List.for_all (fun (k, v) -> let v' = Ctxt.find k s2 in v = v')

    let dtlist_is_subset ~larger ~smaller = 
      let subset = ref true in 
      (* Every dtlist in smaller must be a subset of a dtlist in larger. *)
      for i=0 to List.length smaller - 1 do 
        (* If we haven't yet failed to find a match. *)
        if !subset 
        then begin
          (* Try to find the dt in larger that is a superset. *)
          let (this_dt, _, _) = List.nth smaller i in 
          let found = ref false in 
          for j=0 to List.length larger - 1 do 
            (* For each dt in the superset, check to see if this_dt is a 
               subset. *)
            if not !found
            then begin 
              let (possible_match, _, _) = List.nth larger j in 
              let kvlist = Ctxt.bindings this_dt in 
              (* found := kvlist 
                |> List.for_all (fun (k, v) -> Ctxt.find k possible_match = v) *)
              found := kvlist
                |> List.for_all (fun (k,_) -> Ctxt.mem k possible_match)
            end 
          done;
          subset := !found 
        end
    ...
    let eval_node solver (avoid : Avoid.t ref) declared cfg path dt_list s phis n =
      ...
      | Assert p -> 
        ...
        let consistent_tupes = dt_list 
          |> Utils.filter_map (eval_assert solver !avoid declared path p s phis)
          in 
        (* If any of the conditioning sets grow, check to see if any of the 
           current treatments is a subset of the corresponding treatment. If 
           so, add the current treatment to the list. *)
        let additional = ref [] in  
        (* let added = ref false in 
        for i=0 to List.length consistent_tupes - 1 do 
          let (dt_list', (s', phis')) = List.nth consistent_tupes i in 
          (* TODO: add equals to the Ctxt module *)
          if (not !added) &&
             (not (ctxt_equals s' s)) && 
             (dtlist_is_subset ~larger:dt_list' ~smaller:dt_list) 
          then begin 
           additional := (dt_list, (s, phis))::!additional;
           added := true
          end 
        done; *)
        assert (List.length !additional < 2); 
        (* (dt_list, (s, phis))::consistent_tupes *)
        Utils.uniq (consistent_tupes @ !additional)
      | Declare (k, qs, t) -> 
    ...
    let eval_path ?pathid:(id=(-1)) solver cfg (p : pi) : ((tmt list) * cndst) list =
      Logger.print_info __LINE__ "Evaluating path %d" id;
      SMT.push solver;
    ...
    let conditioned_on_eq (tupe1 : semantic_tuple) (tupe2 : semantic_tuple) = 
      ...
      Sigma.equal (=) s1 s2 && 
        (* unevaluated endogenous variables should not be merged. *)
          Phi.equal sphi1 Phi.empty && 
            Phi.equal sphi2 Phi.empty 
    ...
    let get_sigma_card (lst : cpo_expr Sigma.t list) : (string * int) list = 
      let module M = Map.Make(String) in 
      (* Maps from string to a set of values *)
      let m = ref M.empty in 
      (* Loop over all of the sigmas *)
      for i=0 to List.length lst - 1 do 
        let sigmas : (string * cpo_expr) list = List.nth lst i |> Sigma.bindings in 
        for j=0 to List.length sigmas - 1 do 
          let ((k : string), (v: cpo_expr)) = List.nth sigmas j in 
          if M.mem k !m then 
            let p = M.find k !m in 
            let v = Phi.add v p in 
            m := M.add k v !m
          else 
            m := M.add k (Phi.add v Phi.empty) !m
        done;
      done;
      (* Get the list of variable names and values *)
      M.bindings !m 
      |> List.map (fun (k, lst) -> (k, Phi.cardinal lst))

    let get_sigma_freq (lst : cpo_expr Sigma.t list) : (string * int) list = 
      let module M = Map.Make(String) in 
      let m = ref M.empty in 
      (* Loop over all of the sigmas *)
      for i=0 to List.length lst - 1 do 
        let sigmas : (string * cpo_expr) list = List.nth lst i |> Sigma.bindings in 
        for j=0 to List.length sigmas - 1 do 
          let ((k : string), _) = List.nth sigmas j in 
          if M.mem k !m then 
            let count = M.find k !m in 
            m := M.add k (count + 1) !m
          else 
            m := M.add k 1 !m
        done;
    ...
    let get_phi_freq (lst : Phi.t list) : (Phi.elt * int) list =
      let module M = Map.Make(struct 
        type t = Phi.elt
        let compare a b = Phi.compare (Phi.add a Phi.empty) (Phi.add b Phi.empty)
        end) in 
      let m = ref M.empty in 
      for i=0 to List.length lst - 1 do 
        let (phi : Phi.t) = List.nth lst i in 
        let (elts: Phi.elt list) = Phi.elements phi in 
        for j=0 to List.length elts - 1 do 
          let elt : Phi.elt = List.nth elts j in 
          if M.mem elt !m then 
            let count = M.find elt !m in 
            m := M.add elt (count + 1) !m
          else
            m := M.add elt 1 !m
        done;
      done;
    ...
    (* Remove from conditioning sets or phis anything that is the same for every 
       path. *)
    let remove_subpopulation (sem_tup_list : semantic_tuple list) : semantic_tuple list = 
      (* First find the unique features. *)
      let cond_sets = List.map snd sem_tup_list in 
      let sigma_card = get_sigma_card (List.map fst cond_sets) in 
      let sigma_freq = get_sigma_freq (List.map fst cond_sets) in 
      let phi_freq : (Phi.elt * int) list = get_phi_freq (List.map snd cond_sets) in 
      let tot = List.length sem_tup_list in 
      (* remove things that appear in every cond set and have zero variation *)
      let conds_to_remove : string list = sigma_card
        |> List.filter (fun (k, v) -> v = 1 && List.assoc k sigma_freq = tot) 
        |> List.map fst in 
      (* remove phis that appear in every phi*)
      let phis_to_remove : Phi.elt list = phi_freq
        |> List.filter (fun (_, v) -> v = tot) 
        |> List.map fst in 
      
      sem_tup_list
      |> List.map (fun (tlist, (sigma, phi)) ->
          let new_sigma = List.fold_left 
              (fun sigma s -> Sigma.remove s sigma)
              sigma
              conds_to_remove in 
          let new_phi = List.fold_left 
              (fun (phi : Phi.t) (e : Phi.elt) -> Phi.remove e phi)
              phi
              phis_to_remove in 
          (tlist, (new_sigma, new_phi))
      )
    ...
    let remove_non_dependents_from_conditioning_sets ddg tupes = tupes
      |> List.map (fun (treatment_list, (s, phis)) ->
    ...
    let remove_non_variants labelMap tupes = tupes
      |> List.map (fun (dtlist, (s, phis)) -> 
          let dtlist = dtlist
            |> List.map (fun (dt, r, dtphis) ->
                let randomly_assigned = Ctxt.bindings dt 
                  |> List.filter (fun (k, _) -> 
                      LabelMap.find_rand k labelMap <> Config.Det) in 
                (List.fold_left (fun dt (k, v) -> 
                  Ctxt.add k v dt) Ctxt.empty randomly_assigned, 
                 r, dtphis)
              ) in 
          (dtlist, (s, phis))  
        )
    
    let generate_subsets g (tupes : 't) = 
      (* Do multipass later *)
      let leaves : string list = Cpo.PODdg.get_leaves g |> List.map fst in 
      Printf.printf "LEAVES: %s\n" (Utils.join ", " Utils.identity leaves);
      (* Remove all leaves from treatments *)
      let tupes' : 't = tupes
        |> List.map (fun (dtlist, (s, phis)) ->
            let dtlist' = dtlist
              |> List.map (fun (dt, bar, dtphis) -> 
                  let dt' = List.fold_left 
                    (fun m k -> Delta.remove k m)
                    dt
                    leaves in 
                  let dtphis' = 
                    (* PHelp.bindings dtphis 
                    |> List.fold_left (fun p (k, _) ->
                          if List.mem k leaves 
                          then PHelp.remove_key p k
                          else p)  *)
                      dtphis 
                  in (dt', bar, dtphis')) 
            (* in *)
            (* remove non-dependents from s and phis *)
            (* let ancestors = dtlist'
              |> List.map Utils.fst3 
              |> List.map (fun dt -> Delta.bindings dt)
              |> List.flatten
              |> List.map fst 
              |> List.map (Cpo.PODdg.get_ancestors g)
              |> List.flatten
              |> Utils.uniq in  *)
            (* let s' = Ctxt.bindings s
              |> List.fold_left (fun m (k, _) ->
                    if List.mem k ancestors 
                    then m 
                    else Ctxt.remove k m)
                  s in  *)
            (* let phis' = PHelp.bindings phis 
              |> List.fold_left (fun p (k, _) ->
                    if List.mem k ancestors 
                    then p
                    else PHelp.remove_key p k)
            ...
            in (dtlist, (s, phis))) 
        |> Utils.uniq in
      Printf.printf "Num new subsets: %d\n" (List.length tupes');
       tupes' @ tupes
      (* remove_non_dependents_from_conditioning_sets g tupes *)
    ...
    let make_label_map (p: program) = 
      let labelMap = ref LabelMap.empty in 
      for i=0 to List.length p - 1 do 
        let path = List.nth p i in 
        for j=0 to List.length path - 1 do 
          match List.nth path j with 
          | Assign (_, k, q, _) -> 
            labelMap := LabelMap.add k q !labelMap
          | _ -> ()
        done 
      done; 
      !labelMap
    ...
    let eval cfg g (p: program) : semantic_tuple list = 
      ...
      let labelMap = make_label_map logged_paths in 
      ...
      Logger.print_info __LINE__
        "Logged paths through program: %d\n" (List.length logged_paths);

      let logged_paths_evaled = logged_paths
        |> Utils.enumerate
        |> List.map 
          (fun (pathid, p) -> (p, eval_path ~pathid solver cfg p))  in
      SMT.pop solver;
      ...
      let retval = 
        ...
        |> promote_phis
        (* |> remove_non_dependents_from_conditioning_sets g *)
        |> remove_non_variants labelMap
        |> convert_varidkeys
        |> merge_equivalent_conditioning_sets
        |> remove_constants constants 
        |> remove_subpopulation
        |> generate_subsets g
      in 
      retval 
